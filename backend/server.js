...
app.delete('/api/admin/contact-messages/:id', async (req, res) => {
    try {
        ...
        await pool.request()
            .input('id', sql.Int, id)
            .query('DELETE FROM ContactSubmissions WHERE SubmissionID = @id');
...
app.post('/api/contact/submit', async (req, res) => {
    try {
        ...
        const result = await pool.request()
            ...
            .query(`INSERT INTO ContactSubmissions (Name, Email, Message, SubmissionDate, Status)
                    VALUES (@name, @email, @message, @submissionDate, 'New')`);
