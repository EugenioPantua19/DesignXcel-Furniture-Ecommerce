...
-- Create unique filtered index to ensure only one active discount per product
IF NOT EXISTS (SELECT *
FROM sys.indexes
WHERE name='IX_ProductDiscounts_UniqueActive' AND object_id = OBJECT_ID('ProductDiscounts'))
BEGIN
...
ELSE
BEGIN
    PRINT 'Unique filtered index already exists.';
END
GO
